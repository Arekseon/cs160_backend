...
def extract_audio(input_video_file_name, audio_name):
   ff = ffmpy.FFmpeg(
   inputs={input_video_file_name:None},
...
def accept_post_requests():
   action = request.form['request']



   if action == "add_video_file_to_queue":
      file = request.files['video_file']  
      video_file_name = request.form['video_file_name']
      file.save(video_file_name)
      file.close()
      ...
      first_frame_name = "{}_first_frame.jpeg".format(os.path.splitext(video_file_name)[0])

      get_first_frame(video_file_name, first_frame_name)

      return send_file(first_frame_name)
...
if __name__ == "__main__":
   ...
   #    print("reseaved data: {}".format(data))
